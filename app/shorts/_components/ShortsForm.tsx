...
import { ShortsGenre } from "@/helpers/types";
...
// The create payload sends `visibility` ("public" | "private") but the read model
// carries `statusId`. Prefer an explicit visibility field when the backend returns
// one, and fall back to the status flag the way the comic publish check does.
const deriveVisibility = (short: any): string => {
  if (!short) return "";
  if (typeof short.visibility === "string" && short.visibility) {
    return short.visibility;
  }
  if (Number(short.statusId) === 1) return "public";
  if (Number(short.statusId) === 0) return "private";
  return "";
};

// Filename shown in the preview column when no new file has been picked.
...
export type ShortsFormProps = {
  mode: "create" | "edit";
  /** Required when mode === "edit". Shorts are uuid-keyed, not id-keyed. */
  shortUuid?: string;
};

function ShortsForm({ mode, shortUuid }: ShortsFormProps) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { user, userType, token } = useSelector(selectAuthState);
  const isEdit = mode === "edit";
  const [formStep, setFormStep] = useState(isEdit ? 1 : 0);
  ...
  // An edit URL without an id can't do anything useful - send them back.
  useEffect(() => {
    if (isEdit && !shortUuid) {
      router.push("/user/library");
    }
  }, [isEdit, shortUuid, router]);

  // Fetch the short being edited
  const {
    data: shortResponse,
    isSuccess: isShortSuccess,
    isLoading: isShortLoading,
  } = useQuery({
    queryKey: [`fetch_short_${shortUuid}`],
    queryFn: () =>
      getRequestProtected(
        `my-libraries/shorts/${shortUuid}/get`,
        token,
        prevRoutes().library
      ),
    enabled: isEdit && !!shortUuid && token !== null,
  });

  const shortData = shortResponse?.data;

  // A failed fetch (not yours / deleted) should not leave an empty form on screen.
  useEffect(() => {
    if (isEdit && isShortSuccess && shortResponse && !shortResponse.success) {
      toast(shortResponse.message || "Could not load this short", {
        toastId: "edit_short_load",
        type: "error",
      });
      router.push("/user/library");
    }
  }, [isEdit, isShortSuccess, shortResponse, router]);

  // Show the existing cover as soon as the record lands
  useEffect(() => {
    if (isEdit && isShortSuccess && shortData) {
      setOverlayPreview(shortData.coverImage ?? null);
    }
  }, [isEdit, isShortSuccess, shortData]);
  ...
  const attachedComicId =
    shortData?.comicId ?? shortData?.comic_id ?? shortData?.comic?.id ?? null;

  const initialValues = {
    // the read model calls the video `upload`, the create payload calls it `videoLink`
    url: shortData?.upload || "",
    overlayUrl: shortData?.coverImage || "",
    title: shortData?.title || "",
    description: shortData?.description || "",
    isForKids: shortData?.audienceId ? String(shortData.audienceId) : "",
    visibility: deriveVisibility(shortData),
    comicId: attachedComicId ? String(attachedComicId) : "",
    belongToComic: attachedComicId ? "1" : "0",
    genreId: Array.isArray(shortData?.genres)
      ? shortData.genres.map((item: ShortsGenre) => item.genre_id.toString())
      : ([] as string[]),
  };
    ...
    if (files && files.length > 0) {
      ...
      uploadShortMutation.mutate(file, {
        onSuccess: async (url) => {
          ...
          // In edit mode the creator is already past the picker step, so
          // replacing the video must not shunt them forward.
          if (isEdit) return;
  ...
  const invalidateShorts = () => {
    // We redirect straight to /user/library, so the list mounts moments from
    // now. `invalidateQueries` alone is not enough there: react-query would
    // still hand the mounting component its cached page and refetch in the
    // background, and `LibraryShorts` copies whatever it receives into local
    // state - so the old titles/covers paint first. Dropping the entries
    // instead means the list can only ever render freshly fetched data.
    // `my_shorts` is keyed as ["my_shorts", pagination, tabName]; matching on
    // the prefix clears every page/tab variant of it.
    queryClient.removeQueries({ queryKey: ["my_shorts"] });

    // These aren't being navigated to, so a plain invalidate is right - they
    // refetch whenever the feed/carousel is next visited.
    queryClient.invalidateQueries({ queryKey: ["shorts"] });
    queryClient.invalidateQueries({ queryKey: ["shorts-home"] });
    if (shortUuid) {
      // Not `removeQueries` - this form is still mounted for the 1s before the
      // redirect, and dropping its own record would blank the fields through
      // `enableReinitialize`. Invalidating keeps the values on screen.
      queryClient.invalidateQueries({ queryKey: [`fetch_short_${shortUuid}`] });
      queryClient.invalidateQueries({ queryKey: [`short_${shortUuid}`] });
    }
  };
  ...
  const UpdateShort = useMutation({
    mutationKey: ["patch_short"],
    mutationFn: async (data: any) => {
      const response = await patchRequestProtected(
        data,
        `my-libraries/shorts/${shortUuid}/update`,
        token as string,
        prevRoutes().library,
        "json"
      );
        ...
        <div className="parent-wrap py-10">
          <div className="child-wrap w-full">
            <div className="flex gap-10">
              <BackButton />
              <H2SectionTitle title="Edit Short" />
            </div>
            <p className="text-[#969AA0] pt-10">Loading short…</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-dvh">
      <div className="parent-wrap py-10">
        <div className="child-wrap w-full ">
          <div className="flex gap-10">
            <BackButton />
            <H2SectionTitle title={isEdit ? "Edit Short" : "Upload Shorts "} />
          </div>
          <Formik
            ...
            onSubmit={(values) => {
              ...
              const mutation = isEdit ? UpdateShort : CreateShort;
              const toastId = isEdit ? "edit_short" : "create_short";
              const fallbackMessage = isEdit
                ? "Short updated successfully!"
                : "Short uploaded successfully!";

              mutation.mutate(finaleValues, {
                onSuccess: (response: any) => {
                  // postRequestProtected/patchRequestProtected resolve with a
                  // FailedResponse instead of throwing, so onError never fires
                  // on a backend rejection - check the flag explicitly.
                  ...
                  <div className="bg-none md:bg-[#151D29] h-full  w-full px-[0rem] py-[1.5rem] md:px-[1.5rem] md:py-[2.5rem] lg:px-[4.5rem] lg:py-[3.5rem] ">
                    <div className="flex gap-10 flex-col-reverse md:flex-row">
                      ...
                      <div className="bg-[#151D29] md:bg-none px-[0.8rem] py-[1rem] md:py-0 md:px-0 flex flex-col gap-5">
                        ...
                        {/* Replace the video without leaving the details step */}
                        {isEdit && (
                          <div className="flex flex-col gap-2">
                            <label
                              htmlFor="replaceVideoUpload"
                              className={`${
                                uploadShortMutation.isPending
                                  ? "bg-gray-500 cursor-not-allowed"
                                  : "bg-gradient-to-r from-[#00A96E] to-[#22C55E] cursor-pointer"
                              } text-white px-4 py-2 rounded-md inline-block text-sm w-max`}
                            >
                              {uploadShortMutation.isPending
                                ? "Uploading..."
                                : "Replace video"}
                            ...
                            <input
                              id="replaceVideoUpload"
                              type="file"
                              className="hidden"
                              accept="video/mp4,video/x-m4v,video/*"
                              disabled={uploadShortMutation.isPending}
                              onChange={(e) =>
                                handleFile(
                                  e,
                                  validateForm,
                            ...
                            />
                            <p className="text-xs text-[#969AA0]">
                              Leave this alone to keep the current video. Max
                              50MB.
                            </p>
                          </div>
                        )}
                        ...
                        <p>{selectedFile?.name || fileNameFromUrl(values.url)}</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-none md:bg-[#151D29] h-full md:h-full w-full px-[0rem] py-[1.5rem] md:px-[1.5rem] md:py-[2.5rem] lg:px-[4.5rem] lg:py-[3.5rem]">
                      ...
                      <div className="flex gap-10 flex-col-reverse md:flex-row">
                        ...
                        <div className="flex flex-col w-full gap-5 md:w-1/2 bg-[#151D29] md:bg-none px-[0.5rem] py-[1.5rem] sm:px-[1.5rem] sm:py-[2.5rem] md:px-[0rem] md:py-[0rem]">
                          ...
                          <p>
                            {selectedFile?.name || fileNameFromUrl(values.url)}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between mt-10">
                        ...
                        <button
                          type="submit"
                          disabled={
                            CreateShort.isPending || UpdateShort.isPending
                          }
                          className={`${
                            CreateShort.isPending || UpdateShort.isPending
                              ? "bg-gray-500 cursor-not-allowed"
                              : "bg-gradient-to-r from-[#00A96E] to-[#22C55E]"
